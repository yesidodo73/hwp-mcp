...
기존 버전은 `hwp_mcp_stdio_server.py`가 실행된 프로세스 안에서 바로 HWP COM 자동화를 수행했습니다. 이 방식은 단순하고 빠르지만, Codex/Claude Desktop처럼 MCP 서버가 Windows Job 안에서 실행되는 환경에서는 `Hwp.exe -Automation` 시작 시 `Hnc.Controls.Native.FrameWindowImpl` 오류가 발생하며 실패할 수 있었습니다.

현재 버전은 이 문제를 우회하기 위해 구조가 바뀌었습니다.

- `hwp_mcp_stdio_server.py`: MCP stdio 서버이자 라우터 역할
- `hwp_mcp_helper.py`: 실제 HWP COM 객체를 오래 유지하는 helper
- `hwp_mcp_broker.py`: Windows Job 바깥에서 helper 요청을 받는 외부 broker
- `start_hwp_broker.ps1`: broker를 손쉽게 시작하는 PowerShell 스크립트

정리하면:

- 원버전: `MCP 서버 프로세스 -> 직접 COM 제어`
- 현재 버전: `MCP 서버 -> broker/helper -> COM 제어`

추가로 다음 안정화 작업이 포함되어 있습니다.

- 한글 텍스트 추출 시 깨진 한글(mojibake) 자동 복원
- 보안 모듈 DLL 경로의 프로젝트 상대경로화
- HWP 시작 실패 시 cleanup/cooldown 처리
- stdio 오염을 막기 위한 로깅 정리
- helper/broker 경로를 검증하는 테스트 추가
...
### Codex/Claude Desktop에서 권장 실행 순서

Codex/Claude Desktop처럼 MCP 서버가 별도 앱 내부에서 실행되는 환경에서는, 먼저 외부 broker를 띄운 뒤 MCP 서버를 사용하는 것을 권장합니다.

1. broker 시작:

```powershell
powershell -ExecutionPolicy Bypass -File .\start_hwp_broker.ps1
```

2. 그 다음 Claude/Codex에서 기존과 동일하게 `hwp_mcp_stdio_server.py`를 MCP 서버로 사용

이렇게 하면 MCP 서버가 Windows Job 안에 있더라도 실제 HWP COM 자동화는 Job 바깥 broker가 담당합니다.
...
```
hwp-mcp/
├── hwp_mcp_broker.py        # Job 바깥에서 HWP COM을 유지하는 broker
├── hwp_mcp_helper.py        # broker/stdio 서버가 사용하는 COM helper
├── hwp_mcp_stdio_server.py  # 메인 서버 스크립트
├── start_hwp_broker.ps1     # broker 시작 스크립트
...
### `FrameWindowImpl` 오류 팝업이 뜨는 경우
`The type initializer for 'Hnc.Controls.Native.FrameWindowImpl' threw an exception.` 팝업은 대개 HWP 2024의 자동화 시작(`-Automation`) 경로가 실패할 때 나타납니다.

이 저장소의 현재 버전은 이 문제를 줄이기 위해 외부 broker 구조를 사용합니다. 다음 순서로 확인하세요.

1. 열려 있는 `Hwp`/`HwpApi` 프로세스를 모두 종료
2. `start_hwp_broker.ps1`로 broker 시작
3. Claude/Codex를 다시 시작한 뒤 MCP 호출 재시도

업데이트가 가능한 환경이라면 한컴 업데이트 후 재시도하는 것이 가장 좋지만, 업데이트가 어려운 환경에서는 broker 우회가 현실적인 대안입니다.
...
### 2026-04-01
- HWP 자동화 안정화
  - stdio 서버 직접 COM 제어 구조를 broker/helper 기반 구조로 변경
  - Windows Job 안에서 실행되는 Codex/Claude Desktop 환경 대응
  - `FrameWindowImpl` 자동화 시작 오류 우회 경로 추가
  - helper 연결 실패 시 cleanup/cooldown 로직 추가
  - 텍스트 추출 한글 깨짐 자동 복원 로직 추가
  - broker/helper/연결 경로 테스트 추가
